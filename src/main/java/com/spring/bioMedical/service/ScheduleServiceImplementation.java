...
public class ScheduleServiceImplementation implements ScheduleService {
    ...
    @Override
    public List<AppointmentSlot> listSlots(Long clinicId, LocalDate date) {
        return slotRepo.findByClinicIdAndSlotDateOrderBySlotTimeAsc(clinicId, date);
    }
    ...
    private Long resolveDefaultDoctorId(Long clinicId) {
        return doctorRepo.findTopByClinic_ClinicIdOrderByDoctorIdAsc(clinicId)
                .map(d -> d.getDoctorId())
    ...
    /**
     * Sinh slot theo giờ clinic, bỏ qua khoảng nghỉ (vd 12:00–13:00).
     * KHÔNG truyền doctorId; hệ thống tự gán bác sĩ mặc định của chi nhánh.
     */
    @Transactional
    ...
    public int generateSlotsForDateWithBreak(Long clinicId,
        ...
        for (LocalTime t = open; !t.plusMinutes(minutes).isAfter(close); t = t.plusMinutes(minutes)) {
            ...
            if (!exists) {
                ...
                s.setSlotDate(date);
                s.setSlotTime(t);           // cột TIME ↔ LocalTime
    ...
    // ================== BOOK / CANCEL ==================
    /**
     * Book chuẩn: đánh dấu slot BOOKED nếu đang AVAILABLE, và tạo AppointmentBooking.
     */
    @Override
    @Transactional
    ...
    /**
     * Book có kiểm tra chi nhánh: chỉ cho phép đặt khi slot thuộc đúng clinic của actor.
     */
    @Override
    @Transactional
    public AppointmentBooking bookIntoClinic(Long userId, Long slotId, String note, Long clinicIdOfActor) {
        ...
        slotRepo.findBySlotIdAndClinicId(slotId, clinicIdOfActor)
                .orElseThrow(() -> new IllegalStateException(
